...
#[derive(Error, Debug)] // implement the error trait from thiserror and debug trait from std
#[error("{msg}")] // This is a custom error message that will be displayed if the error is thrown
...
fn parse_card_numbers(card: &str) -> Result<Vec<u32>, ParsePaymentInfoError> {
    let numbers = card
        .split(" ")
        .map(|s| {
            s.parse().with_context(|| format!("{s:?} could not be parsed as u32")) // converts to a anyhow error
        })
...
enum CreditCardError {
    #[error("{0}")] // Display trait from thiserror
    InvalidInput(String),
    #[error(transparent)]
    Other(#[from] anyhow::Error), // source error from anyhow
}
...
fn get_credit_card_info(
    ...
    let card = parse_card(card_string)
        .with_context(|| format!("{name}'s card could not be parsed.")) // converts to a anyhow error
        .map_err(|e| {
