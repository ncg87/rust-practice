...
fn find_subarray(nums: &[i32], sum: i32) -> (usize, usize) {
    for len in (1..nums.len() + 1).rev() {
        for start in 0..nums.len() - len + 1 {
            // iterates trhough all subarrays brute force
...
// we aren't modifying the array, so we can take a slice
fn array_sum(nums: &[i32]) -> i32 {
    let mut res = 0;
    for num in nums {
        res += num; // sum the elements of the array over all the elements in slice
    }
    res // pass the sum back and ownership
}
